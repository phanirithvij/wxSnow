...
class CollisionDetector(object):
    ...
    def get_collisions(self):
        ...
        self.y_list.sort(key=functools.cmp_to_key(CollisionDetector.cmp_y))
        xset = set()
        yset = set()
        n = len(self.objects)
        for i1 in range(n):
...
class Frame(wx.Frame):
    def __init__(self, handle):
        ...
        w, h = self.GetSize()
        self.w, self.h = w, h
        self.bitmaps = [wx.Bitmap('images/flake%d.png' % n)
                        for n in range(1, 9)]
        self.flakes = []
        self.create_flakes()
        self.update()
    ...
    def create_flakes(self):
        ...
        flakes = [Flake(self.w, self.h, self.bitmaps[random.randint(0, 7)])
                  for i in range(n)]
...
class TaskBarIcon(wx.adv.TaskBarIcon):
    ...
    def CreatePopupMenu(self):
        ...
        if SPEED == SPEEDS[-1]:
            item.Enable(False)
        item = menu_item(menu, 'Slower', self.on_slower,
                         icon='images/delete.png')
        if SPEED == SPEEDS[0]:
            item.Enable(False)
        menu.AppendSeparator()
        if ROTATE:
            menu_item(menu, "Don't Rotate", self.on_rotate,
                      icon='images/arrow_down.png')
        else:
            menu_item(menu, "Rotate", self.on_rotate,
                      icon='images/arrow_rotate_clockwise.png')
        menu.AppendSeparator()
        menu_item(menu, 'Exit', self.on_exit, icon='images/door_out.png')
        return menu

    def set_icon(self, image_file, tooltip='wxSnow'):
        icon = wx.Icon(wx.Bitmap(image_file))
...
if __name__ == '__main__':
    ...
    handle = 0
    handle = handle or find_window(
        desktop, ['Progman', 'SHELLDLL_DefView', 'SysListView32'])
    handle = handle or find_window(
        desktop, ['WorkerW', 'SHELLDLL_DefView', 'SysListView32'])
    app = wx.App()
    frame = Frame(handle)
    app.MainLoop()
